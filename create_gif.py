# USAGE: python create_gif.py --config config.json --image images/raghav.jpg --output out.gif
...
def overlay_image(bg, fg, fgMask, coords):
	(sH, sW) = fg.shape[:2] #get foreground spatial dimensions (width and height)
	(x, y) = coords #point where the foreground will be placed

	overlay = np.zeros(bg.shape, dtype="uint8") #overlay has same dimensions as background image and is totally blank (i.e. 0s)
	overlay[y:y + sH, x:x + sW] = fg #except for the foreground which is added to the overlay using array slicing

	# the alpha channel controls *where* and *how much* transparency a given region has
	alpha = np.zeros(bg.shape[:2], dtype="uint8") #alpha channel has same width & height as background image and is blank (i.e. 0s)
	alpha[y:y + sH, x:x + sW] = fgMask #but it contains foreground mask
	alpha = np.dstack([alpha] * 3)

	output = alpha_blend(overlay, bg, alpha) #perform alpha blending to merge  foreground, background and alpha channel together
...
# With It" sunglasses to so let's find the detection with the largest probability
...
# approximation and use 90% of the face width for the sunglasses width
...
for (i, y) in enumerate(steps):
	...
	# if this is the final step then we need to add the "DEAL WITH IT" text to the bottom of the frame
	if i == len(steps) - 1:
